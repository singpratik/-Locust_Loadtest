...
class InterviewUser(HttpUser):
    ...
    def execute_interview_flow(self):
        """Execute the complete interview flow with sequential UID processing for 2 questions."""
        ...
        expected_questions = 2  # Modified to expect only 2 questions
    ...
    def create_mock_interview(self):
        """Create a mock interview with only 2 questions."""
        if not self.access_token:
        ...
        # Modified to request only 2 questions
        payload = {
            "template_id": 9205,
            "difficulty": "Basic",
            "language": "en",
            "selected_questions": [123385, 123386]  # Only 2 questions
        }
        ...
        with self.client.post(
            ...
            if response.status_code in [200, 201]:
                ...
                if len(self.available_uids) != 2:
                    logger.error(f"Expected 2 questions but got {len(self.available_uids)}")
                    return False
